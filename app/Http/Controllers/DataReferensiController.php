...
class DataReferensiController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = DataReferensi::with('media');
        ...
        if ($connection === 'sqlite') {
            $bulanTahunList = DataReferensi::selectRaw("strftime('%Y-%m', tanggal) as bulan_tahun")
                ->distinct()
                ->pluck('bulan_tahun');

            $tahunList = DataReferensi::selectRaw("strftime('%Y', tanggal) as tahun")
                ->distinct()
                ->pluck('tahun');
        } else {
            $bulanTahunList = DataReferensi::selectRaw("DATE_FORMAT(tanggal, '%Y-%m') as bulan_tahun")
                ->distinct()
                ->pluck('bulan_tahun');

            $tahunList = DataReferensi::selectRaw("YEAR(tanggal) as tahun")
                ->distinct()
                ->pluck('tahun');
        }

        // List tanggal buat filter manual (kalau mau dipakai)
        $tanggalList = DataReferensi::select('tanggal')->distinct()->get();
        return view('SIWAS.dokumen.data_referensi', compact('dokumens', 'tanggalList', 'bulanTahunList', 'tahunList'));
    }
