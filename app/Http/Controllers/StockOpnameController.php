...
class StockOpnameController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($connection === 'sqlite') {
            $bulanTahunList = StockOpname::selectRaw("strftime('%Y-%m', tanggal) as bulan_tahun")
                ->distinct()
                ->pluck('bulan_tahun');

            $tahunList = StockOpname::selectRaw("strftime('%Y', tanggal) as tahun")
                ->distinct()
                ->pluck('tahun');
        } else {
            $bulanTahunList = StockOpname::selectRaw("DATE_FORMAT(tanggal, '%Y-%m') as bulan_tahun")
                ->distinct()
                ->pluck('bulan_tahun');

            $tahunList = StockOpname::selectRaw("YEAR(tanggal) as tahun")
                ->distinct()
                ->pluck('tahun');
        }

        // List tanggal buat filter manual (kalau mau dipakai)
        $tanggalList = StockOpname::select('tanggal')->distinct()->get();;
        return view('SIWAS.dokumen.stock_opname', compact('dokumens', 'tanggalList', 'bulanTahunList', 'tahunList'));
    }
