...
class DokumenController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($connection === 'sqlite') {
            $bulanTahunList = Dokumen::selectRaw("strftime('%Y-%m', tanggal) as bulan_tahun")
                ->distinct()
                ->pluck('bulan_tahun');

            $tahunList = Dokumen::selectRaw("strftime('%Y', tanggal) as tahun")
                ->distinct()
                ->pluck('tahun');
        } else {
            $bulanTahunList = Dokumen::selectRaw("DATE_FORMAT(tanggal, '%Y-%m') as bulan_tahun")
                ->distinct()
                ->pluck('bulan_tahun');

            $tahunList = Dokumen::selectRaw("YEAR(tanggal) as tahun")
                ->distinct()
                ->pluck('tahun');
        }

        // List tanggal buat filter manual (kalau mau dipakai)
        $tanggalList = Dokumen::select('tanggal')->distinct()->get();
        return view('SIWAS.dokumen.dokumen_spi', compact('dokumens', 'tanggalList', 'bulanTahunList', 'tahunList'));
    }
