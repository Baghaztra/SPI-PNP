...
class RKAKLController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($connection === 'sqlite') {
            $bulanTahunList = RKAKL::selectRaw("strftime('%Y-%m', tanggal) as bulan_tahun")
                ->distinct()
                ->pluck('bulan_tahun');

            $tahunList = RKAKL::selectRaw("strftime('%Y', tanggal) as tahun")
                ->distinct()
                ->pluck('tahun');
        } else {
            $bulanTahunList = RKAKL::selectRaw("DATE_FORMAT(tanggal, '%Y-%m') as bulan_tahun")
                ->distinct()
                ->pluck('bulan_tahun');

            $tahunList = RKAKL::selectRaw("YEAR(tanggal) as tahun")
                ->distinct()
                ->pluck('tahun');
        }

        // List tanggal buat filter manual (kalau mau dipakai)
        $tanggalList = RKAKL::select('tanggal')->distinct()->get();
        return view('SIWAS.dokumen.rkakl', compact('dokumens', 'tanggalList', 'bulanTahunList', 'tahunList'));
    }
